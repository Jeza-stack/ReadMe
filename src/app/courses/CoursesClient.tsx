...
export default function CoursesClient({ courses }: { courses: CatalogueCourse[] }) {
  ...
  const tabs = ['All Results', 'Core English', 'Professional', 'Workshops'];

  const filterOptions = {
    subject: ['Core English', 'Professional', 'Workshops'],
    level: ['A1 - Beginner', 'A2 - Elementary', 'B1 - Intermediate', 'B2 - Upper Intermediate', 'All Levels'],
    duration: [...new Set(courses.map((c) => c.duration))],
    difficulty: [...new Set(courses.map((c) => c.difficulty))],
  };
      ...
      <section className="relative pt-24 pb-32 overflow-hidden bg-[linear-gradient(135deg,#021A42_0%,#043370_50%,#00A2C9_100%)] text-white">
        ...
        <div className="container relative mx-auto px-4 md:px-8 z-10 max-w-6xl">
          ...
          <div className="text-cyan-100 text-lg mb-8 font-medium">
            1 - {filteredCourses.length} of {courses.length} search results
          </div>
