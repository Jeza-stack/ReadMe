...
export default function B1Level() {
  const grammarTopics = [
    { topic: 'Present Perfect vs Past Simple', lessons: 7, href: '/level/b1/grammar/present-perfect' },
    { topic: 'Conditional Sentences (1st & 2nd)', lessons: 6, href: '/level/b1/grammar/conditionals' },
    { topic: 'Passive Voice (Present & Past)', lessons: 5, href: '/level/b1/grammar/passive-voice' },
    { topic: 'Reported Speech', lessons: 6, href: '/level/b1/grammar/reported-speech' },
    { topic: 'Relative Clauses', lessons: 5, href: '/level/b1/grammar/relative-clauses' },
    { topic: 'Modal Verbs (Advanced)', lessons: 4, href: '/level/b1/grammar/modal-verbs' }
  ];

  const vocabularyThemes = [
    { theme: 'Abstract Concepts & Ideas', words: 50 },
    { theme: 'Technology & Media', words: 45 },
    { theme: 'Environment & Nature', words: 40 },
    { theme: 'Society & Culture', words: 55 },
    { theme: 'Education & Learning', words: 35 },
    { theme: 'Emotions & Relationships', words: 40 }
  ];
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="text-center mb-12 animate-in fade-in slide-in-from-top-4 duration-700">
          <Badge className="bg-[#00A2C9] text-white text-lg px-6 py-2 mb-6 rounded-full shadow-lg shadow-cyan-400/20">B1 Level</Badge>
          <h1 className="font-headline text-4xl md:text-6xl font-bold mb-6 text-slate-900 dark:text-white leading-tight">Intermediate English</h1>
            ...
            <div className="grid md:grid-cols-3 gap-8 text-center md:text-left">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-xs font-bold text-[#00A2C9]">0/265 words</span>
                </div>
                <Progress value={0} className="h-2 bg-slate-100 dark:bg-white/10" />
              </div>
              <div className="space-y-3">
        ...
        <Tabs defaultValue="grammar" className="space-y-10">
                ...
                {vocabularyThemes.map((theme, index) => (
                   <Card key={index} className="border border-slate-100 dark:border-white/5 hover:border-[#00A2C9] transition-all bg-white dark:bg-[#0A1330] rounded-xl shadow-sm">
                      <CardHeader>
                        ...
                        <CardDescription>{theme.words} intermediate words</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <Button className="w-full font-semibold" variant="outline">Learn Concepts</Button>
                      </CardContent>
                   </Card>
                ))}
             </div>
          ...
          <TabsContent value="assessment" className="animate-in fade-in duration-500 outline-none">
             <Card className="border-2 border-dashed border-slate-200 dark:border-white/10 bg-transparent text-center p-12 rounded-3xl">
                ...
                <h3 className="text-2xl font-bold mb-4 text-slate-800 dark:text-white">B1 Proficiency Exam</h3>
                <p className="text-slate-500 max-w-md mx-auto mb-8">Ready to move to B2? Complete all intermediate benchmarks to unlock the final certification.</p>
        ...
        <div className="mt-20 flex justify-between items-center py-8 border-t border-slate-200 dark:border-white/10">
           <Link href="/level/a2" className="flex items-center gap-2 text-slate-500 hover:text-[#043370] font-bold transition-colors group">
             <ArrowLeft className="w-5 h-5 group-hover:-translate-x-1 transition-transform" /> Previous: A2 Elementary
           </Link>
           <Link href="/level/b2" className="flex items-center gap-2 text-[#00A2C9] hover:text-[#043370] font-bold transition-colors group">
             Next Up: B2 Upper-Intermediate <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
           </Link>
        </div>
      </div>
    </div>
